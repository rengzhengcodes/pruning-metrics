...
def test_apply_wanda_pruning_ratio_zero_is_identity(tiny_model) -> None:
    ...
    apply_wanda_pruning(tiny_model, _build_stats(tiny_model), prune_ratio=0.0)
    assert torch.equal(tiny_model.layer.weight.data, snapshot)


def test_apply_wanda_pruning_handles_huge_score_tensor() -> None:
    ...
    apply_wanda_pruning(model, stats, prune_ratio=0.2)
